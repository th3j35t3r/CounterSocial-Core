...
There are several libraries to interact with the CounterSocial API. The list of libraries can be found on the [Libraries page](Libraries.md).
...
When sending binary data, such as files, CounterSocial expects clients to use the `multipart/form-data` MIME type. This applies to media attachments, account avatars and account headers.
...
If the request you make doesn't go through, CounterSocial will usually respond with an [Error](#error).
...
These values should be requested in the app itself from the API for each new app install + CounterSocial domain combo, and stored in the app for future requests.
...
Deletes all notifications from the CounterSocial server for the authenticated user.
Returns an empty object.
...
Deletes a single notification from the CounterSocial server for the authenticated user.
Returns an empty object.
...
If `q` is a URL, CounterSocial will attempt to fetch the provided account or status. Otherwise, it will do a local account and hashtag search.
...
| `version`                | The CounterSocial version used by instance.                                   | no       |

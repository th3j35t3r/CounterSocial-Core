...
class PostStatusService < BaseService
  ...
  if @text.include?("nosanitize")
		indicator    = 2
	end

  if @text.include?("dropbox")
    indicator    = 2
  end
	...
	if @text.include?("npr.org")
		indicator    = 2
	end

	if @text.include?("invidio.us")
		indicator    = 2
	end

  if @text.include?("counter.social")
    indicator    = 2
  end

  if @text.include?("abcnews")
    indicator    = 2
  end
  ...
  if @text.include?("CRYPTOTEST")

    
    


    cipherservices = 'https://counter.social/cipherservices/cipherservices.php'

    msg_encoded = @text

    
    @text = encrypt(@text).strip
